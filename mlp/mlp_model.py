...
def inference(images, num_classes, for_training=False, restore_logits=True,
	...
	with tf.variable_scope(scope, 'MultiLayerPerceptron', [images]):
		# Layer 1: (32x32x3) ->     (16x16x160)
		with tf.variable_scope('layer1'):
		    x = _conv(images, 160, 2, 2, scope='fc1')
		    x = _relu(x, leakiness=0.0)
		    x = _batch_norm(x, scope='batchnorm1', is_training=for_training)
		    _activation_summary(x)
                
		# Layer 2: (16x16x160) ->   (1x1x1000)
		with tf.variable_scope('layer2'):
			x = _conv(x, 1000, 16, 1, padding='VALID', scope='fc1')
			x = _relu(x, leakiness=0.0)
			x = _batch_norm(x, scope='batchnorm1', is_training=for_training)
			_activation_summary(x)

		# Layer 3: (1x1x1000) ->    (1x1x1000)
		with tf.variable_scope('layer3'):
			x = _conv(x, 1000, 1, 1, scope='fc1')
			x = _relu(x, leakiness=0.0)
			x = _batch_norm(x, scope='batchnorm1', is_training=for_training)
			_activation_summary(x)
